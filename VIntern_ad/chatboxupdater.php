				...
				<div>
				  ...
				  ?>
					...
					if($_SESSION['user_type_name'] == 5){
						...
						$select = dbQuery("SELECT * FROM tbl_user  WHERE  (tbl_user.user_type !=0 AND tbl_user.user_type !=0 AND  tbl_user.user_type !=1 AND tbl_user.user_id != $user_id );");
						dbNumRows($select);				
						
					}elseif($_SESSION['user_type_name'] != 4){
						$select = dbQuery("SELECT * FROM tbl_user WHERE tbl_user.user_type !=0 AND ((SELECT tbl_student.group_id FROM tbl_student WHERE tbl_student.group_id = '$group_id' AND tbl_student.user_id= tbl_user.user_id) OR tbl_user.user_type = 4 OR (tbl_user.user_type = 5)) AND tbl_user.user_id != $user_id;");// AND 
						
						
					}else{
						$select = dbQuery("SELECT * FROM tbl_user  WHERE  tbl_user.user_type !=0 AND tbl_user.user_id != $user_id OR (tbl_user.user_type = 5);");
						dbNumRows($select);
					}
					$total_unread = 0;
					...
					<div class="row" <?php echo @$style; ?>>	
						<div class="col-md-6" >
							...
							?>
							<center><?php echo getUserType($row['user_type']); ?></center>
						</div>
						<div class="col-md-6">
